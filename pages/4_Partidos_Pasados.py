...
with past:
    st.markdown(f"<h2 style='text-align: center;'>Partidos pasados</h2>", unsafe_allow_html=True)
    st.write('###')
    global_stage = ''

    if results.empty:
        st.write('###')
        st.markdown(f"<h4 style='text-align: center;'>No hay partidos pasados</h4>", unsafe_allow_html=True)
    else:
